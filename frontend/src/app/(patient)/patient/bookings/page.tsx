  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="text-2xl font-bold text-slate-900">Lịch hẹn của tôi</h2>
        <p className="text-sm text-slate-500">Theo dõi lịch hẹn và trạng thái xử lý.</p>
      </header>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="grid grid-cols-12 border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-500">
        ...
        {isLoading ? (
            ...
            {data.map((b) => (
              <div className="grid grid-cols-12 gap-2 px-5 py-4 text-sm" key={b.id}>
                <div className="col-span-3">
                  <p className="font-bold text-slate-900">{b.bookingCode}</p>
                  <p className="text-xs text-slate-500">
                    Tạo lúc {new Date(b.createdAt).toLocaleString('vi-VN')}
                  </p>
                </div>
                <div className="col-span-3">
                  <p className="font-semibold text-slate-900">{b.doctorName}</p>
                </div>
                <div className="col-span-3">
                  <p className="font-semibold text-slate-900">{b.specialtyName}</p>
                </div>
          ...
          <div className="p-5 text-sm text-slate-600">Chưa có lịch hẹn nào.</div>
        )}
      </div>
    </div>
  );
